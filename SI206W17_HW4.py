...
for x in soup.find_all(class_="field field-name-title field-type-ds field-label-hidden"):
	name = x.string
	if name is not None:
		t = str(name).strip()
		names.append(t)

for x in soup.find_all(class_="field field-name-field-person-titles field-type-text field-label-hidden"):
	title = x.string
