...
def get_user_tweets(username):
	...
	tweet_status = []
	for twt in twitter_results:
		tweet_status.append(twt)
	return tweet_status[:20]
...
lst = []
for single_tweet in umsi_tweets:
	y = single_tweet
	z  = (y["id"], y["user"]["screen_name"], y["created_at"], y["text"], y["retweet_count"])
	lst.append(z)
...
class PartTwo(unittest.TestCase):
	...
	def test7(self):
		self.assertTrue(set([int(x[-1]) > 2 for x in more_than_2_rts]) in [{},{True}])
